  ...
  loadInitialDataForChart() {
    this.salesService.salesList$.subscribe({
      next: (sales) => {
        ...
        salesStages.reduce((map, stage) => {
          map[stage] = sales
            .filter((sale) => sale.salesStage === stage)
            .reduce((sum, sale) => sum + sale.estimatedValue, 0);
          return map;
    ...
    this.reportRes$.subscribe({
      next: (report: SalesReport) => {
        this.pieChartData = {
          ...
          data: [
            report.dataPoints.opportunitiesInProgress,
            report.dataPoints.opportunitiesWon,
            report.dataPoints.opportunitiesLost,
          ],
        };
      },
