...
export class SalesService {
  ...
  updateSales(salesOpportunity: SalesOpportunity) {
    return this.httpClient
      .put<SalesOpportunity>(environment.apiUrl + '/sales-opportunity/' + salesOpportunity.opportunityID, salesOpportunity)
      .pipe(
        tap((updatedLead) => {
          const updatedSales = this.salesSubject.value.map((sale) =>
            sale.opportunityID === updatedLead.opportunityID ? updatedLead : sale
          );
  ...
  updateNotificationSchedule(cron: string) {
    return this.httpClient.post<{ id: string; taskName: string; cronExpression: string }>(
      environment.apiUrl + '/sales-opportunity/setReminderSchedule',
      { cronExpression: cron }
    );
  }
  ...
  updateClosingSchedule(cron: string) {
    return this.httpClient.post<{ id: string; taskName: string; cronExpression: string }>(
      environment.apiUrl + '/sales-opportunity/setClosingSchedule',
      { cronExpression: cron }
    );
  }
